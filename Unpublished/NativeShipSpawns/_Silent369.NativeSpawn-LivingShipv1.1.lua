local modfilename = "NativeSpawn-LivingShip"
...
local RobotSpawnWeighting = 0  --Original "0" sentinel (robot), fighters spawn regularly
local SpawnFrequencyValue = 5  --Spawn Frequencies within T1-T3 and Outlaw economies.
------------------------------------------------------------------------------------------------------
...
-- Check if ShipSpawnerData is nil or empty
-- if not then call GcAISpaceshipModelDataArray()
if ShipSpawnerData and #ShipSpawnerData > 0 then
    table.insert(NMS_MOD_DEFINITION_CONTAINER.MODIFICATIONS, {MBIN_CT = GcAISpaceshipModelDataArray()})
end
