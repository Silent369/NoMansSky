local modfilename = "AlmostPerfectFlightNoReverse"
local lua_author  = "Silent"
local lua_version = "5.7"
...
--Ship Boosts
local m_boostMult = 1.5
local m_boostMxSp = 2.0
local m_max_speed = 2.5
local m_thrstMult = 1.5
local m_warpSMult = 2.5
...
local function insertEngineData(controlType, engineType, controlMult)
    local entry = {
        SKW = {controlType, "GcPlayerSpaceshipControlData", engineType, "GcPlayerSpaceshipEngineData"},
        MATH_OP = "*",
        REPLACE_TYPE = "ALL",
        VCT = {
            {"ThrustForce",        m_thrstMult * controlMult},
            {"Falloff",            m_speedFall * controlMult},
            {"LowSpeedTurnDamper", m_lsptrnDmp * controlMult},
            {"MaxSpeed",           m_max_speed * controlMult},
            {"BoostThrustForce",   m_boostMult * controlMult},
            {"BoostMaxSpeed",      m_boostMxSp * controlMult},
            {"OverspeedBrake",     m_overBreak * controlMult},
            {"DirectionBrakeMin",  m_dBreakMin * controlMult},
            {"DirectionBrake",     m_dBreakMax * controlMult},
            {"ReverseBrake",       m_rev_break * controlMult},
            {"TurnStrength",       m_trnStreng},
            {"TurnBrakeMin",       m_turnBrMin * controlMult},
            {"TurnBrakeMax",       m_turnBrMax * controlMult},
            {"RollAmount",         m_rolAmount * controlMult},
            {"RollForce",          m_rollForce * controlMult},
            {"RollAutoTime",       m_rollATime * controlMult},
            {"BalanceTimeMin",     m_baltimMin * controlMult},
            {"BalanceTimeMax",     m_baltimMax * controlMult}
        }
    }
...
if m_shipStatsBonus then
    for _, controlType in ipairs(controlTypes) do
        local controlMult = controlMulti[controlType][1]
        for _, engineType in ipairs(engineTypes) do
            insertEngineData(controlType, engineType, controlMult)
        end
    end
end
